...
public class PortExtraction extends VerilogParserBaseVisitor<Void> {
    ...
    public Void visitModule_instantiation(VerilogParser.Module_instantiationContext ctx) {
        System.out.println("Instantiating:" + ctx.getChild(0).getChild(0).getChild(0).getChild(0).getText());
    ...
    private AttributeProps visitAttribute_collectProps(VerilogParser.Attribute_instanceContext ctx) {
        ...
        for (VerilogParser.Attr_specContext sc : ctx.attr_spec()) {
            String an = sc.attr_name().getText();
            if (an.equals("analog") && (sc.constant_expression() == null)) {
                ap.isAnalog = true;
            } else if (an.equals("side") && (sc.constant_expression() != null)) {
                if (sc.constant_expression().constant_primary() == null ||
                        (sc.constant_expression().constant_primary().identifier() == null)) {
                    throw new RuntimeException("Side attribute must be a constant identifier (UP, DOWN, LEFT, RIGHT, BACK, FRONT).");
                }
                String sideText = sc.constant_expression().constant_primary().identifier().getText();
                int sideInd = directionNames.indexOf(sideText);
                if (sideInd == -1) {
                    throw new RuntimeException("Side " + sideText + " not recognized. Must be one of: " + directionNames);
                }
    ...
    private void visitConstantExpression_Color(VerilogParser.Constant_expressionContext ctx, AttributeProps ap) {
        ...
        if (cp_ctx.identifier() != null) {
            String colorText = cp_ctx.identifier().getText();
            ...
            List<VerilogParser.Constant_expressionContext> exprs = cp_ctx.constant_concatenation().constant_expression();
    ...
    public Void visitReg_declaration(VerilogParser.Reg_declarationContext ctx) {
        ...
        VerilogParser.Variable_typeContext vt = lic.variable_type().get(0);
    ...
    public static int computeRangeSize(VerilogParser.Range_Context r) {
        if (r == null) {
            ...
            VerilogParser.Constant_expressionContext ub = r.msb_constant_expression().constant_expression();
            VerilogParser.Constant_expressionContext lb = r.lsb_constant_expression().constant_expression();
    ...
    public Void visitInput_declaration(VerilogParser.Input_declarationContext ctx) {
        VerilogParser.Port_identifierContext identifier = ctx.list_of_port_identifiers().port_identifier(0);
    ...
    private void visitInputOutputDeclaration(boolean isInput, VerilogParser.Range_Context rctx, VerilogParser.Port_identifierContext pctx) {
        if (propsTracker == null) {
            throw new RuntimeException("Expected module port declaration to have an attribute declaration with color or list of colors, and side direction.");
        ...
        if (rs != ip.colors.size()) {
            throw new RuntimeException("Size of port (" + rs + ") does not match amount of colors specified (" + ip.colors.size() + ").");
        }
